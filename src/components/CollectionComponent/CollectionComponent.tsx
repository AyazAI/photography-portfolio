...
const CollectionComponent: React.FC<CollectionComponentProps> = ({
  ...
  const updateUrlFilter = (newFilter: string) => {
    ...
    window.history.replaceState({}, '', `${pageContext.urlPathname}?${params}`);
  };
  const showFilter =
    pageContext.urlPathname === '/work' || pageContext.urlPathname === '/photo';
  ...
  const renderImageGridBlock = (b: CollectionBlockDB) => {
    ...
    return (
      <Wrapper key={b.id} $itemsCount={items.length} $aspectRatio={aspectRatio}>
        {items.map((item, i) => (
          <div key={i}>
            <img
  ...
  const renderBlock = (b: CollectionBlockDB) => {
    switch (b.type) {
      ...
      case 'TEXT_TITLE': {
        ...
        return (
          <COLLECTION_TEXT_TITLE_WRAPPER key={b.id} align={align}>
            <COLLECTION_TEXT_TITLE as={style} fontSize={fontsize} align={align}>
              {text}
            </COLLECTION_TEXT_TITLE>
          </COLLECTION_TEXT_TITLE_WRAPPER>
        );
      ...
      case 'SPLITTER_DEFAULT':
        return (
          <hr key={b.id} style={{ margin: '20px 0', borderColor: '#444' }} />
        );
      case 'SPLITTER':
  ...
  return (
    <CollectionContainer>
      ...
      {blocks.map(b => (
        <Reveal key={b.id}>{renderBlock(b)}</Reveal>
      ))}
